...
_disable_qcawificfg80211() {
	...
	case "$chk" in				
		...
		ath*)
			...
			disable_qcawificfg80211_vap $device $chk
		;;
	esac
...
enable_qcawificfg80211_guest_vap() {
...
enable_qcawificfg80211_vap() {
	local device="$1"
	local chk="$2"
	
	if [ "$chk" == "$DEV24G" ] || [ "$chk" == "$DEV5G" ]; then
		config_get vifs "$device" vifs
	else
		vifs=$chk
	fi

	echo vifs $vifs >> /tmp/wireless_backend
	...
	for vif in $vifs; do
		...
		config_get ngvhtintop "$vif" ngvhtintop
		[ -n "$ngvhtintop" ] && "$device_if" "$ifname" 11ngvhtintop "$ngvhtintop"
		...
		case "$ext_nss" in
			1|on|enabled) "$device_if" "$device" ext_nss 1 >/dev/null 2>&1
				;;
			0|on|enabled) "$device_if" "$device" ext_nss 0 >/dev/null 2>&1
				;;
			*) ;;
		esac
...
#TODO
		...
		if [ "$type" == "guest" ]; then
			...
			guestLan=br-$network
			enable_qcawificfg80211_guest_vap $vif
		...
		license_status=$(zapi-cli -s "6")
		if [ "$license_status" == "0" ] || [ "$license_status" == "5" ]; then
			ethtool -K $vif gro off
		else
			ethtool -K $vif gro on
		fi
	done
}

qcawificfg80211_set_channel() {
	local device="$1"
	local vif="$2"
	local channel="$3"
	local ifname=$vif
	...
	if [ "$hwmode" == "11axa" ] && [ "$channel_width" == "Auto" ]; then
		htmode=HT160
	fi
	## (Zyxel) end ##
...
enable_qcawificfg80211_radio() {
	...
	"$device_if" "$device" setCountryID "$countryid"
	...
	config_get he_bsscolor_override "$device" he_bsscolor_override 0
	[ -n "$he_bsscolor" ] && "$device_if" "$phy" he_bsscolor ${he_bsscolor} ${he_bsscolor_override}

	config_get self_obss_pd_tx_enable "$device" self_obss_pd_tx_enable
	[ -n "$self_obss_pd_tx_enable" ] && "$device_if" "$phy" self_obss_pd_tx_enable ${self_obss_pd_tx_enable}

	if [ "$self_obss_pd_tx_enable" == "1" ]; then
		config_get self_obss_pd_tx_threshold "$device" self_obss_pd_tx_threshold
		[ -n "$self_obss_pd_tx_threshold" ] && "$device_if" "$phy" set_self_obss_pd_tx_threshold ${self_obss_pd_tx_threshold}
	fi
	...
	for vif in $vifs; do
		...
		if [ -z "$recover" ] || [ "$recover" -eq "0" ]; then
		    wlanconfig "$vif" create wlandev "$phy" wlanmode "$pmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon} -cfg80211

		    echo "cfg80211: ifname: $vif mode: $wlanmode cfgphy: $(cat /sys/class/net/$phy/phy80211/name)" >&2
		    iw phy "$(cat /sys/class/net/$phy/phy80211/name)" interface add $vif type $wlanmode
		    [ $? -ne 0 ] && {
			echo "enable_qcawifi($device): Failed to set up $mode vif $vif" >&2
			continue
		    }
		    config_set "$vif" ifname "$vif"

			## (Zyxel) set channel ##
			qcawificfg80211_set_channel $device $vif $channel
			## (Zyxel) end of set channel ##
		fi

		[ $qwrap_enable -gt 0 ] && iw "$vif" set 4addr on >/dev/null 2>&1

		## (Zyxel) set vap ##
		enable_qcawificfg80211_vap $device $vif
...
enable_qcawificfg80211() {
	...
	line_num=$(cat /tmp/wireless_backend | wc -l)

	if [ ${line_num} -gt 200 ];then
		date > /tmp/wireless_backend
	else
		date >> /tmp/wireless_backend
	fi
	...
	echo "Start ----- Enable device=$device" >> /tmp/wireless_backend
	...
	echo reload $chk >> /tmp/wireless_backend

	if [ $chk == $DEV24G ] || [ $chk == $DEV5G ]; then
		enable_qcawificfg80211_radio $device $chk
	else
		config_get disabled $chk disabled

		echo disabled $disabled >> /tmp/wireless_backend

		if [ "$disabled" == "0" ]; then
			if [ ! -f /sys/class/net/$chk ]; then
				create_qcawificfg80211_vap $device $chk
			fi
			enable_qcawificfg80211_vap $device $chk
		fi
	fi
