...
DEV5G_2_FIRSTNAME="ath2"	#ath20 ath21 ath22 ath23


chk_24G_Apply="/tmp/wifi24G_Apply"						#ath10 ath11 ath12 ath13
chk_24G_Status="/tmp/wifi24G_Status"
chk_24G_Status_number="/tmp/wifi24G_Status_number"		#0:idle 1:isSetting
echo 0 > $chk_24G_Status_number

chk_5G_Apply="/tmp/wifi5G_Apply"						#ath0 ath1 ath2 ath3
chk_5G_Status="/tmp/wifi5G_Status"
chk_5G_Status_number="/tmp/wifi5G_Status_number" 		#0:idle 1:isSetting
echo 0 > $chk_5G_Status_number

chk_5G_2_Apply="/tmp/wifi5G_2_Apply"						#ath20
chk_5G_2_Status="/tmp/wifi5G_2_Status"
chk_5G_2_Status_number="/tmp/wifi5G_2_Status_number" 		#0:idle 1:isSetting
...
scan_qcawifi() {
	...
	## (Zyxel) set vifs ##
	uci set wireless.$device.zy_vap_list="$vifs"
	## (Zyxel) end of set vifs ##
...
load_qcawifi() {
	...
	if [ "$mp_256" == "MP_256" ]; then
		...
		sysctl -w dev.nss.n2hcfg.extra_pbuf_core0=4400000 >/dev/null 2>/dev/null
		sysctl -w dev.nss.n2hcfg.n2h_high_water_core0=38816 >/dev/null 2>/dev/null
		sysctl -w dev.nss.n2hcfg.n2h_wifi_pool_buf=16384 >/dev/null 2>/dev/null
	else
	case "$board_name" in
	ap-hk09*)
	...
	;;
	ap-oak*)
		#total pbuf size is 160 bytes,allocate memory for 32120 pbufs
		sysctl -w dev.nss.n2hcfg.extra_pbuf_core0=5200000 >/dev/null 2>/dev/null
		sysctl -w dev.nss.n2hcfg.n2h_high_water_core0=43840 >/dev/null 2>/dev/null
		sysctl -w dev.nss.n2hcfg.n2h_wifi_pool_buf=8192 >/dev/null 2>/dev/null
	;;
	ap-hk*|nbg7815)
		hk_ol_num="$(cat /lib/wifi/wifi_nss_hk_olnum)"
		if [ $hk_ol_num -eq 2 ]; then
			[ -d /sys/module/qca_ol ] || { \
				#total pbuf size is 160 bytes,allocate memory for 39288 pbufs
				sysctl -w dev.nss.n2hcfg.extra_pbuf_core0=6300000 >/dev/null 2>/dev/null
				sysctl -w dev.nss.n2hcfg.n2h_high_water_core0=51008 >/dev/null 2>/dev/null
				sysctl -w dev.nss.n2hcfg.n2h_wifi_pool_buf=24576 >/dev/null 2>/dev/null
				}
		elif [ $hk_ol_num -eq 3 ]; then
			#total pbuf size is 160 bytes,allocate memory for 48504 pbufs
			sysctl -w dev.nss.n2hcfg.extra_pbuf_core0=7800000 >/dev/null 2>/dev/null
			sysctl -w dev.nss.n2hcfg.n2h_high_water_core0=60224 >/dev/null 2>/dev/null
			sysctl -w dev.nss.n2hcfg.n2h_wifi_pool_buf=24576 >/dev/null 2>/dev/null
		fi
	;;
	*)
...
check_ar71xx_24Gstatus(){
	local chk="$(cat $chk_24G_Status)"
	if [ $chk == "wifi24GisSetting" ];then
		echo "The wifi24G is setting....Please wait!!!!" > /dev/console
		echo 1 > $chk_24G_Status_number
		return 0
	fi
	return 1
}

check_ar71xx_5Gstatus(){
	local chk="$(cat $chk_5G_Status)"
	if [ $chk == "wifi5GisSetting" ];then
		echo "The wifi5G is setting....Please wait!!!!" > /dev/console
		echo 1 > $chk_5G_Status_number
		return 0
	fi
	return 1
}

check_ar71xx_5G_2status(){
	local chk="$(cat $chk_5G_2_Status)"
	if [ $chk == "wifi5G_2_isSetting" ];then
		echo "The wifi5G 2 is setting....Please wait!!!!" > /dev/console
		echo 1 > $chk_5G_2_Status_number
		return 0
	fi
	return 1
}
...
disable_qcawifi_vap(){
# This function disables specific VAP interfaces
# This is similar to disable_ar71xx_vap function in WSQ50
    ...
    [ -f /sys/class/net/${vap}/parent ] && { \
        ...
        [ -n "$parent" -a "$parent" = "$device" ] && {
			...
			local type
			config_get type "$vap" type

			echo "$DRIVERS disable vap $vap" >/dev/console

			if [ "$type" == "main" ]; then
				ubus call network.interface.lan remove_device "{ \"name\": \"$vap\" }"
			elif [ "$type" == "guest" ]; then
				ubus call network.interface.guest remove_device "{ \"name\": \"$vap\" }"
			elif [ "$type" == "visitor" ]; then
				ubus call network.interface.visitor remove_device "{ \"name\": \"$vap\" }"
			elif [ "$type" == "backhaul" ]; then
				ubus call network.interface.lan remove_device "{ \"name\": \"$vap\" }"
			fi
...
_disable_qcawifi() {
	...
	if [ "$device" == "$DEV24G" ];then
		check_ar71xx_24Gstatus && return 0

		if [ ! -e $chk_24G_Apply ];then
			local chk=$DEV24G
		else
			local chk="$(cat $chk_24G_Apply)"
		fi

		##set wifi24g status
		echo "wifi24GisSetting" > $chk_24G_Status
	elif [ "$device" == "$DEV5G" ];then
		check_ar71xx_5Gstatus && return 0

		if [ ! -e $chk_5G_Apply ];then
			#local chk="$DEV5G_FIRSTNAME"0
			local chk=$DEV5G
		else
			local chk="$(cat $chk_5G_Apply)"
		fi

		##set wifi5g status
		echo "wifi5GisSetting" > $chk_5G_Status
	else
		check_ar71xx_5G_2status && return 0

		if [ ! -e $chk_5G_2_Apply ];then
			local chk=$DEV5G_2
		else
			local chk="$(cat $chk_5G_2_Apply)"
		fi

		##set wifi5g 2 status
		echo "wifi5G_2_isSetting" > $chk_5G_2_Status		
	fi
	...
	case "$chk" in				
		"$DEV24G")
			...
			echo "disable 2.4G"
			disable_qcawifi_radio $@
			echo "wifi24Gisidle" > $chk_24G_Status
		;;
		"$DEV5G")
			##### disable 5G
			echo "disable 5G"
			disable_qcawifi_radio $@
			echo "wifi5Gisidle" > $chk_5G_Status
		;;
		"$DEV5G_2")
			##### disable 5G 2
			echo "disable 5G 2"
			disable_qcawifi_radio $@
			echo "wifi5G_2_isidle" > $chk_5G_2_Status
		;;
		ath*)
			#####disable interface 
			echo "disable interface $chk"
			disable_qcawifi_vap $device $chk
		;;
	esac
...
enable_qcawifi_guest_vap(){
# This enables guest-related functions
# This is similar to enable_ar71xx_guestWlan and guest_wlan in WSQ50
	local vif=$1

	local system_mode=$(uci_get system main system_mode)
	local op_role=$(uci get system.main.operation_role)
	if [ "$system_mode" != "2" ] || [ "$op_role" != "controllee" ]; then
		...
		tc qdisc del dev $vif root 2>/dev/null
		tc qdisc del dev fc root 2>/dev/null
		tc qdisc del dev pppoe-wan root 2>/dev/null
		tc qdisc del dev pptp-wan root 2>/dev/null	
		
		bandwidth_manage=$(uci_get wireless $vif guest_bandwidth_enable)
		if [ "$bandwidth_manage" == "1" ]; then
			max_bandwidth=$(uci_get wireless $vif guest_max_bandwidth)
			max_bandwidth=$max_bandwidth"kbit"
			
			tc qdisc add dev $vif root handle 10: htb default 20
			tc class add dev $vif parent 10: classid 10:1 htb rate $max_bandwidth ceil $max_bandwidth
			tc class add dev $vif parent 10:1 classid 10:20 htb rate 1kbit ceil $max_bandwidth prio 1
			tc qdisc add dev $vif parent 10:20 handle 102: pfifo limit 10000
		fi
...
enable_qcawifi_vap() {
	...
	if [ "$chk" == "$DEV24G" ] || [ "$chk" == "$DEV5G" ] || [ "$chk" == "$DEV5G_2" ]; then
		config_get vifs "$device" vifs
	else
		vifs=$chk
	fi

	echo $vifs >> /tmp/wireless_backend
	...
	for vif in $vifs; do
		...
		## (Zyxel) assign ifname ##
		config_set $vif ifname $vif
		## (Zyxel) end of assign ifname ##
		...
		if [ "$type" == "guest" ]; then
			...
			#ubus call network.interface.visitor add_device "{ \"name\": \"$vif\" }"
				
			#base_addr=$(ifconfig ath1 | grep HWaddr | awk '{print $5}')
			#ifconfig $guestLan hw ether $base_addr	
		elif [ "$type" == "visitor" ]; then
			config_get network $vif network
			visitorLan=br-$network
			enable_qcawifi_guest_vap $vif

			if [ "$visitor_network_feature" == "2" ]; then
				bridge_interface visitor
				
				local system_mode=$(uci_get system main system_mode)
				if [ "$system_mode" == "1" ]; then
					ipaddr=$(uci get network.visitor.ipaddr)
					ipmask=$(uci get network.visitor.netmask)
					ifconfig $visitorLan $ipaddr netmask $ipmask up
				else
					ifconfig $visitorLan up
				fi
		...
		## (Zyxel) end of set guest, visitor vap ##
...
qcawifi_set_channel() {
	...
	## (Zyxel) Not include 11ax yet ##
	case "$channel_width" in
		160) htmode=HT160 ;;
		80_80) htmode=HT80_80 ;;
		80) htmode=HT80 ;;
		20) htmode=HT20 ;;
		*|40)	
	...
	## (Zyxel) end of not include 11ax yet ##
	...
	[ $pureg -gt 0 ] && iwpriv "$vif" pureg "$pureg"

	config_get cfreq2 "$vif" cfreq2
	[ -n "$cfreq2" -a "$htmode" = "HT80_80" ] && iwpriv "$vif" cfreq2 "$cfreq2"

	config_get puren "$vif" puren
	[ -n "$puren" ] && iwpriv "$vif" puren "$puren"

	iwconfig "$vif" channel "$channel" >/dev/null 2>/dev/null
}

enable_qcawifi_radio() {
	local device="$1"
	local chk="$2"
	
	echo "$DRIVERS enable radio $device" >/dev/console
	...
	case "$countrycode" in
		ff|FF) countryid="843" ;;
		e1|E1|ee|EE|ce|CE)
			country=$(uci get wireless.wifi1.country)
			case "$country" in
				ch|CH) countryid="756" ;;
				tw|TW) countryid="158" ;;
				ca|CA) countryid="124" ;;
	...
	iwpriv "$device" setCountryID "$countryid"
	
	## (Zyxel) end of country ID ##
	...
	## (Zyxel) get vifs ##
	#vifs="$DEV24G_FIRSTNAME""0 ""$DEV24G_FIRSTNAME""1 ""$DEV24G_FIRSTNAME""3 ""$DEV24G_FIRSTNAME""4"
	#if [ "$device" == "$DEV5G" ]; then
	#	vifs="$DEV5G_FIRSTNAME""0 ""$DEV5G_FIRSTNAME""1 ""$DEV5G_FIRSTNAME""3 ""$DEV5G_FIRSTNAME""4"
	#fi
	
	config_get vifs "$device" zy_vap_list
	## (Zyxel) end of get vifs ##
	...
	*)
	...
	macaddr=$(cat /tmp/AR71XX_24G.dat | awk -F '=' '{print $2}')
	if [ "$device" == "$DEV5G" ]; then
		macaddr=$(cat /tmp/AR71XX_5G.dat | awk -F '=' '{print $2}')
	elif [ "$device" == "$DEV5G_2" ]; then
		macaddr=$(cat /tmp/AR71XX_5G_2.dat | awk -F '=' '{print $2}')
	fi
	iwpriv "$phy" setHwaddr "$macaddr"
	...
	for vif in $vifs; do
		...
		if [ -z "$recover" ] || [ "$recover" -eq "0" ]; then
			ifname=$(/usr/sbin/wlanconfig "$vif" create wlandev "$phy" wlanmode "$wlanmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon})
			[ $? -ne 0 ] && {
		...
		## (Zyxel) set channel ##
		qcawifi_set_channel $device $vif $channel
		## (Zyxel) end of set channel ##
	done
	
	## (Zyxel) set vap ##
	enable_qcawifi_vap $device $chk
	## (Zyxel) end of set vap ##
	...
	## Copy from WSQ50
	#Fix ath0 ath1 ath2 ath3 enable,but ath0 ath1 ath2 not work
	for vif in $vifs
	do
		config_get disabled $vif disabled
		
		if [ "$disabled" == "0" ]; then
			echo "Start enable_qcawifi_radio -- $vif down --" >> /tmp/wireless_backend
			ifconfig $vif down
		fi
	done
	...
	do
		...
		if [ "$disabled" == "0" ]; then
			echo "Start enable_qcawifi_radio -- $vif up --" >> /tmp/wireless_backend
			ifconfig $vif up
		fi
	done
}
...
enable_qcawifi() {
	...
	lock /var/run/wifilock
	## (Zyxel) set wifi radio ##
	if [ "$device" == "$DEV24G" ];then
		if [ "$(cat $chk_24G_Status_number)" == "1" ];then
			echo 0 > $chk_24G_Status_number
			return 0
		fi
		##set wifi24g status
		...
		if [ "$(cat $chk_5G_Status_number)" == "1" ];then
			echo 0 > $chk_5G_Status_number
			return 0
		fi
		##set wifi5g status
		...
		if [ "$(cat $chk_5G_2_Status_number)" == "1" ];then
			echo 0 > $chk_5G_2_Status_number
			return 0
		fi
		##set wifi5g 2 status
		echo "wifi5G_2_isSetting" > $chk_5G_2_Status
	...
	echo $chk >> /tmp/wireless_backend

	if [ $chk == $DEV24G ] || [ $chk == $DEV5G ] || [ $chk == $DEV5G_2 ]; then
		enable_qcawifi_radio $device $chk
	else
		config_get disabled $chk disabled

		echo $disabled >> /tmp/wireless_backend
	...
	if [ "$device" == "$DEV24G" ];then
		echo "$device" > $chk_24G_Apply
		echo "wifi24Gisidle" > $chk_24G_Status
	elif [ "$device" == "$DEV5G" ];then
		echo "$device" > $chk_5G_Apply
		echo "wifi5Gisidle" > $chk_5G_Status
	else
		echo "$device" > $chk_5G_2_Apply
		echo "wifi5G_2_isidle" > $chk_5G_2_Status		
	fi
	
	## (Zyxel) end of set wifi radio ##
	lock -u /var/run/wifilock
}
...
ftm_qcawifi() {
	...
	echo -n "/ini" > /sys/module/firmware_class/parameters/path
	case "$board_name" in
	ap-hk*|ap-dk*|ap-ac*|ap-oa*|nbg7815)
